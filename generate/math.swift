...
let mathFunctions = [
  // trigonometry
  ["name": "cos", "name_acc":  "vvcos",  "doc": "cosine function",     "initialValues": [0.0, 1.57080, 3.14159]],
  ["name": "sin", "name_acc":  "vvsin",  "doc": "sine function",       "initialValues": [0.0, 1.57080, 3.14159]],
  ["name": "tan", "name_acc":  "vvtan",  "doc": "tangent function",    "initialValues": [0.0, 1.57080, 3.14159]],
  ["name": "acos", "name_acc": "vvacos", "doc": "arccosine function",  "initialValues": [1.0, 0.5, -0.25, -0.75]],
  ["name": "asin", "name_acc": "vvasin", "doc": "arcsine function",    "initialValues": [1.0, 0.5, -0.25, -0.75]],
  ["name": "atan", "name_acc": "vvatan", "doc": "arctangent function", "initialValues": [1.0, 0.5, -0.25, -0.75]],
  // hyperbolic
...
func generateFunctionSpec(type: String, data: [String: Any]) -> String {
  ...
  let newStorageComputed = values.enumerated().map { (i, value) in
    "          expect(abs(newStorage[\(i)] - <name>(\(values[i]))) < 0.0001).to.beTrue"
  }
  ...
  let originalStorageUpdated = values.enumerated().map { (i, value) in
    "          expect(abs(storage[\(i)] - <name>(\(values[i]))) < 0.0001).to.beTrue"
  }
  let template = """
      context(\"<doc>\") {
        before {
